...
import {convertDegree2Radian, getDistance, getMove, getNumInRange, limitNumInRange, velocityDecomposition} from '../common/util.js';
...
export default class Player extends PIXI.extras.AnimatedSprite{
    ...
    get collisionCircle() {
        return {
            center: {x: this.frameX, y: this.frameY}, radius: this.radius,
        }
    }
    ...
    shoot() {
        ...
        bullet.reset({
            ...
            direction: this.frameRotation, speed: 0.7, x: this.shootPoint.x, y: this.shootPoint.y,
        });
    ...
    get shootPoint() {
        ...
        return {
            x: this.x + half * Math.cos(this.rotation), y: this.y + half * Math.sin(this.rotation),
        }
    }
